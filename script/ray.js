class Ray {
    ...
    show() {
        ...
        line(this.pos.x, this.pos.y, this.pos.x + this.dir.x * 25, this.pos.y + this.dir.y * 25);
        // line(0, 0, 100, 100);
        //console.log(this.pos)
    ...
    Cast(balls, walls) {
        ...
        for (let i = 0; i < balls.length; i++) {
            newBalls.push(balls[i].body);
        }
        var newWalls = [];

        //console.log(walls);
        for (let i = 0; i < walls.length; i++) {
            newWalls.push(walls[i].body);
            //console.log(walls);
        }
        ...
        //     stroke(255);
        //     line(this.pos.x, this.pos.y, points[q].point.x, points[q].point.y);
        // }
        if (points.length > 0) {
            stroke(255);
            line(this.pos.x, this.pos.y, points[0].point.x, points[0].point.y);
        }


        //stuff to highlight hit objects
        

        var objects = Query.ray(newCast, start, end )

        var distances = [];
        for (var i = 0; i < objects.length; i++) {
            var pos = objects[i].body.position;
            var distX = this.pos.x-pos.x;
            var distY = this.pos.y-pos.y;
            //console.log(pos);
            distances[i] = sqrt((distX*distX)+(distY*distY));
            objects[i].dist = distances[i];
        } 
        distances = sort(distances, distances.length);

        //console.log(distances);

        for (var i = 0; i < objects.length; i++) {
            if (distances[0] == objects[i].dist) {
                objects[i].body.ray = true;
